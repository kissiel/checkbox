...
class CheckboxTouchApplication(PlainboxApplication):
    ...
    def get_next_test(self):
        ...
        """
        dyn_todo_list = self.assistant.get_dynamic_todo_list()
        if self.index < len(dyn_todo_list):
            job = self.assistant.get_job(dyn_todo_list[self.index])
            description = ""
            ...
            test = {
                ...
                "test_number": dyn_todo_list.index(job.id),
                "tests_count": len(dyn_todo_list),
                "command": job.command,
    ...
    def register_test_result(self, test):
        ...
        job_id = test['id']
        job = self.assistant.get_job(job_id)
        builder_kwargs = {
            'outcome': test['outcome'],
            'comments': test.get('comments', pod.UNSET),
            'io_log_filename': test['result'].io_log_filename,
    ...
    def run_test_activity(self, test):
        ...
        res_builder = self.assistant.run_job(
            test['id'], 'silent', test['plugin'] in plugins_handled_natively)
